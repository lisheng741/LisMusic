...
{
    public class KGMusicService : MusicHttpClient, IALLMusicService
    {
        ...
        public KGMusicService(HttpClient httpClient, IConfiguration configuration) : base(httpClient)
        {
        ...
        public async Task<List<MusicInfo>> GetMusicListAsync(string keyWord)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < jo["data"]["lists"].Count(); i++)
                {
                    ...
                    MusicInfo musicInfo = new MusicInfo() { Name = jo["data"]["lists"][i]["SongName"].ToString(), Singer = jo["data"]["lists"][i]["SingerName"].ToString(), Album = jo["data"]["lists"][i]["AlbumName"].ToString(), DownloadInfo = jo["data"]["lists"][i]["FileHash"].ToString(), CanDownload = bolCanDownload, MusicSource = MusicSource.KG };
