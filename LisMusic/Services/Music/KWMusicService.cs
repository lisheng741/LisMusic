...
{
    public class KWMusicService : MusicHttpClient, IALLMusicService
    {
        ...
        public KWMusicService(HttpClient httpClient) : base(httpClient)
        {
        ...
        public async Task<List<MusicInfo>> GetMusicListAsync(string keyWord)
        {
            ...
            try
            {
                ...
                await base.GetAsync(url1);
                string strKwToken = base.GetCookie("kw_token");
                //第二步，获取 列表
                ...
                for (int i = 0; i < jo["data"]["list"].Count(); i++)
                {
                    ...
                    MusicInfo musicInfo = new MusicInfo() { Name = jo["data"]["list"][i]["name"].ToString(), Singer = jo["data"]["list"][i]["artist"].ToString(), Album = jo["data"]["list"][i]["album"].ToString(), DownloadInfo = jo["data"]["list"][i]["musicrid"].ToString(), CanDownload = bolCanDownload, MusicSource = MusicSource.KW };
