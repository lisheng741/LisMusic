        ...
        public async Task<List<MusicInfo>> GetMusicListAsync(string keyWord)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < jo["data"]["song"]["list"].Count(); i++)
                {
                    ...
                    {
                        Name = jo["data"]["song"]["list"][i]["name"].ToString(),
                        Subheading = jo["data"]["song"]["list"][i]["lyric"].ToString(),
                        Artist = jo["data"]["song"]["list"][i]["singer"][0]["name"].ToString(),
                        Album = jo["data"]["song"]["list"][i]["album"]["name"].ToString(),
                        DownloadInfo = jo["data"]["song"]["list"][i]["mid"].ToString(),
                        CanDownload = bolCanDownload,
                        MusicSource = MusicSource.QQ
                    };
