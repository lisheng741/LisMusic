function zplayer(option) {
    ...
    p.isMobile && (p.autoPlay = !1); //如果是手机，则不自动播放
    var y, cur_m = 0, random_cur_m = 0, m = "", ht = "", playType = 0, curPlayType = 0, playTypes = ["z-icon-retweet", "z-icon-retweet-one", "z-icon-random", "z-icon-reorder-list"], playList = [], playListRandom = [];
    ...
    ht += '<div class="zplayer">'
        ...
        + '				  <span class="zplayer-title">' + p.musics[0].Name + '</span>'
        + '				  <span class="zplayer-author"> - ' + p.musics[0].Singer + '</span>'
        + '			    </div>'
        + '			</div>';
    if (!p.fixed) {
    ...
    ht += '			<div class="zplayer-controller">'
        ...
        + '						<i class="icon-previous z-previous"></i>'
        + '						<i class="icon-play z-play"></i>'
        + '						<i class="icon-stop z-stop display-none"></i>'
        + '						<i class="icon-next z-next"></i>'
        + '					</div>'
    ...
    p.PlayButton = p.element.getElementsByClassName("z-play")[0]; //z-icon-play
    p.PauseButton = p.element.getElementsByClassName("z-stop")[0]; //z-icon-pause
    ...
    p.bw = p.element.getElementsByClassName("zplayer-menu-bars")[0].getElementsByClassName("z-previous")[0]; //z-icon-backward
    p.fw = p.element.getElementsByClassName("zplayer-menu-bars")[0].getElementsByClassName("z-next")[0]; //z-icon-forward
    ...
    if (!p.isMobile) {
        p.volumeThumb.addEventListener("mousedown", function () {
            ...
            document.addEventListener("mousemove", voe); //鼠标点下
            document.addEventListener("mouseup", voa);   //鼠标离开
        });
    }
    if (!p.isMobile) {
    ...
    p.autoPlay && play(); //chrome66禁止自动播放，将不会生效
    ...
    function playSwitch(i) {
        ...
        p.element.getElementsByClassName("zplayer-title")[0].innerHTML = p.musics[i].Name;
        p.element.getElementsByClassName("zplayer-author")[0].innerHTML = " - " + p.musics[i].Singer;
